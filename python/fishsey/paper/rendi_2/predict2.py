...
if __name__ == "__main__":
    ...
    for sparess in [5, 10, 15, 20]:
        ...
        for fileNum in range(1, 11):  
            ...
            testFile = "/root/AAA/dataset/rendi2/test/sparseness%s/test%d.txt" % (sparess, fileNum)         
            
            #load dataset
            trainArray = pp.createArrayObj(trainFile)
        
            testData = pp.loadTest(trainFile)
            trainProb =  "/root/AAA/dataset/rendi2/train/sparseness%s/training%d-prob" % (sparess, fileNum)  
            
#            testData = pp.loadTest(testFile)             
#            trainProb =  "/root/AAA/dataset/rendi2/test/sparseness%s/test%d-prob" % (sparess, fileNum)         
...
#                print index
                ...
                if puiProbCoo is None or puiProbEu is None:
#                    temp.append([user, ws, tui])
#                    continue
            ...
            euiResult.append([pow(eui / count, 1.0/pows), count])
            #temp-sparess-fileNum: the remain qos in test to be predicted
            np.savetxt("/root/AAA/dataset/rendi2/temp/tempTrain-%d-%d" % (sparess, fileNum), np.array(temp, dtype=str), fmt='%s', delimiter='\t')
#        np.savetxt("/root/AAA/dataset/rendi2/eui/eui-%d" % (sparess), np.array(euiResult, dtype=str), fmt='%s', delimiter='\t')
