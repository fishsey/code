...
def generateAllPuiArray(sparseness = 5, i = 1):
    fileName = r'rt/pui/slopeone-%d-%d.txt' % (sparseness, i)
    result = np.loadtxt(fileName).reshape(339, 5825)
    paper.save(result, 'rt/allpuiArray-%d-%d.txt' % (sparseness, i)) 
...
def learningAddIndicateFunctionlfm(n, alpha, lamd):
    ...
    puiArray = paper.load(fileNameAllPuiArray)
#    #0-1
#    means = np.mean(puiArray)
#    stdVar = np.std(puiArray)
#    puiArray = (puiArray - means)/ stdVar
    import random
    for step in range(0, n):
        number = 0
        mae = 0
...
if __name__ == '__main__':
    ...
    for number in range(1, fileNums + 1):
        ...
        fileNameAllPuiArray = 'rt/allpuiArray-%d-%d.txt' % (sparseness, number)
