...
#    minSim = 0.55
    ...
    for i in range(1, fileNumbers+1):
        #文件对象
        print i
        euiFileName = 'throught/pui/slopeone-%d-%d.txt' % (sparseness,i)
        ...
        wsAverageVector = paper.columnAverageArray(trainArrayObj, mean) 
         #相似度矩阵数据
        userSimFileName = 'throught/user-%d-%d.txt' % (sparseness,i)
        wsSimFileName = 'throught/ws-%d-%d.txt' % (sparseness,i)
