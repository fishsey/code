...
#    from sklearn.model_selection import GridSearchCV
    import numpy as np
    mm = MinMaxScaler()
    trainX = mm.fit_transform(train[:, :-1])
    testX = mm.fit_transform(test[:, :-1])
    clf = MLPClassifier(activation='logistic', learning_rate='adaptive')
    clf.fit(trainX, train[:, -1].astype(float).astype(int))    

    testProb = clf.predict_proba(testX)    
...
#             train data
#==============================================================================
            trainFile = "dataset/rendi2/train/sparseness%d/training%d.txt" % (sparess, fileNum)
...
#            testFile = "dataset/rendi2/puiAnalyze/puiAnalyze-%d-%d" % (sparess, fileNum)             
            testFile = "dataset/rendi2/test/sparseness%d/test%d.txt" % (sparess, fileNum)    
