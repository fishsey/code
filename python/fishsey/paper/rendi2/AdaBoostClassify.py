...
#    #testFeature
#    userFreq2 = np.zeros((userNum, 21))
#    wsFreq2 = np.zeros((wsNum, 21))
#    for index, line in enumerate(trainData):
#        if flag == True and index % sparess != 0:
#            continue
#        user, ws, tui = line
...
#        ws = int(ws)
#        labelIndex = int(tui) + 1 #labelIndex i means label i-1
#        userFreq2[user, labelIndex] += 1
#        wsFreq2[ws, labelIndex] += 1
...
#            testProb = generateClassifyProb(trainFeature, testFeature)
            result = []
            for k in [10, 20, 30, 50, 100, 150, 200, 300, 400, 500]:
                
                testProb = generateClassifyProb(trainFeature, testFeature, n_estimators=k)
                print k
                result.append(testProb)
#        break
