package _temp._mybatis.plugin.ABTableCache.impl;

import _temp._mybatis.plugin.ABTableCache.EnhancedCachingManager;
...
public class EnhancedCachingExecutor implements Interceptor
{
    ...
    protected Object processQuery(Invocation invocation) throws Throwable
    {
        ...
        if (cachingManager.isCacheEnabled())
        {
            ...
            //如果本条statementId表示的查询语句配置了 flushCache=true，则清空 queryCacheOnCommit缓存
            if (mappedStatement.isFlushCacheRequired())
            {
                queryCacheOnCommit.clear();
            }

            //如果本条statementId表示的查询语句配置了使用缓存，并且二级缓存不为空，则将 StatementId 和对应的二级缓存对象映射关系添加到全局缓存映射管理器中
