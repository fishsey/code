					...
					<tbody>
						<c:forEach var="goods" items="${goodslist}">
							<tr>
								...
								<td>${goods.title}</td>
								<td>${goods.price}</td>
								<td><c:if tools.test="${goods.state==0}">已下架</c:if> <c:if tools.test="${goods.state==1}">销售中</c:if></td>
								<td>${goods.number}</td>
...
<script src="http://apps.bdimg.com/libs/jquery.cookie/1.4.1/jquery.cookie.temp.js"></script>
<script src="<%=path%>/resource/script/handler.temp.js" type="text/javascript"></script>
